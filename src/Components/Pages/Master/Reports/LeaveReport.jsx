...
const columnMapping = {
  'LEAVE APPLIED ON': 'leaveAppliedDate',
  'USER NAME': 'entityName',
  'USER CODE': 'entityCode',
  'ROLE': 'roleName',
  'REPORTING TO': 'reportingAuthority',
  'STATE': 'stateName',
  'CITY': 'cityName',
...
const LeaveReport = () => {
  ...
  const handleSort = (columnName) => {
    ...
    const dataField = columnMapping[columnName];
    if (!dataField) return;

    const sortedRows = [...filteredRows].sort((a, b) => {
      let aValue = a[dataField];
      let bValue = b[dataField];
      ...
      // Handle date fields
      if (dataField === 'leaveAppliedDate' || dataField === 'leaveFromDate' || dataField === 'leaveToDate') {
        aValue = new Date(aValue);
        bValue = new Date(bValue);
      }

      // Handle numeric fields
      if (dataField === 'balanceLeave') {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.reportLink) {
        ...
        setTitle(response.statusMessage || "Excel file downloaded successfully");
        setTimeout(() => {
          setStatus(null);
          setTitle("");
        }, 5000);
        ...
        setStatus(response.statusCode || 400);
        setTitle(response.statusMessage || "Failed to generate Excel file");
      }
    } catch (error) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {status && (
            <Grid item xs={12} ml={2} mb={2} mr={4}>
              {" "}
              <StatusModel
                width="100%"
                status={status}
