...
const IspSaleReport = () => {
  const [activeTab, setActiveTab] = React.useState("isp-sale-report");
  const tabs = [
    { label: "ISP Sales Report", value: "isp-sale-report" },
  ...
  const handleSearch = (searchValues) => {
    const filtered = rows.filter((row) => {
      return (
        ...
        (!searchValues.state || 
          row.column3.toLowerCase().includes(searchValues.state.toLowerCase())) &&
        (!searchValues.city || 
          row.column2.toLowerCase().includes(searchValues.city.toLowerCase())) &&
        (!searchValues.product || 
          row.column7.toLowerCase().includes(searchValues.product.toLowerCase()))
      );
    });
  ...
  const handleSearchClick = () => {
    const searchValues = {
      ...
      product: document.querySelector('[placeholder="Select"]')?.lastValue || "",
    };
