...
const RankingReport = () => {
  const [activeTab, setActiveTab] = React.useState("ranking-report");
  ...
  const generateDummyData = () => {
    const roles = ["RSM", "ASM", "TSM", "ISP"];
    const names = ["John Smith", "Emma Wilson", "Michael Brown", "Sarah Davis", "James Miller"];
    const retailerNames = ["ABC Electronics", "XYZ Mobile", "Tech Zone", "Mobile World", "Phone Hub"];
    const retailerCodes = ["RET001", "RET002", "RET003", "RET004", "RET005"];
    const nations = ["India", "USA", "UK", "Australia", "Canada"];
    const regions = ["North", "South", "East", "West", "Central"];
    const areas = ["Urban", "Rural", "Suburban", "Metropolitan", "Township"];
    ...
    return Array(50).fill().map((_, index) => ({
      ...
      retailerCode: retailerCodes[Math.floor(Math.random() * retailerCodes.length)],
      nation: nations[Math.floor(Math.random() * nations.length)],
      nationalRank: Math.floor(Math.random() * 100) + 1,
      region: regions[Math.floor(Math.random() * regions.length)],
      regionalRank: Math.floor(Math.random() * 50) + 1,
      area: areas[Math.floor(Math.random() * areas.length)],
      areaRank: Math.floor(Math.random() * 25) + 1
    }));
  };
            ...
            <Table sx={{ minWidth: 650 }} size="small" stickyHeader>
              <TableHead>
                ...
                <TableRow sx={{ backgroundColor: LIGHT_GRAY2 }}>
                  {[
                    ...
                    { id: "nationalRank", label: "NATIONAL RANK" },
                    { id: "region", label: "REGION" },
                    { id: "regionalRank", label: "REGIONAL RANK" },
                    { id: "area", label: "AREA" },
                    { id: "areaRank", label: "AREA RANK" }
                  ].map(({ id, label }) => (
                    <TableCell
                      key={id}
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row, index) => (
                    <TableRow key={row.id}>
                      <TableCell sx={{ ...rowstyle }}>{row.role}</TableCell>
                      ...
                      <TableCell sx={{ ...rowstyle }}>{row.nation}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.nationalRank}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.region}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.regionalRank}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.area}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.areaRank}</TableCell>
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
