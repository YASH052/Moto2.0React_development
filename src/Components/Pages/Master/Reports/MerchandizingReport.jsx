...
const MerchandizingReport = () => {
  ...
  const generateDummyData = () => {
    ...
    const codes = ["MR001", "MR002", "MR003", "MR004", "MR005"];
    const storeCodes = ["ST001", "ST002", "ST003", "ST004", "ST005"];
    const tasks = ["Store Audit", "Display Check", "Inventory Count", "Price Verification", "Promotion Setup"];
    ...
    return Array(50)
      ...
      .map((_, index) => ({
        ...
        taskAssigned: tasks[Math.floor(Math.random() * tasks.length)],
        completed: Math.floor(Math.random() * 100),
        pending: Math.floor(Math.random() * 50),
        reopen: Math.floor(Math.random() * 10)
      }));
  };
                ...
                {filteredRows
                  ...
                  .map((row, index) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell sx={{ ...rowstyle }}>{row.storeCode}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.taskAssigned}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.completed}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.pending}</TableCell>
                      <TableCell sx={{ ...rowstyle }}>{row.reopen}</TableCell>
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
...
export default MerchandizingReport;
